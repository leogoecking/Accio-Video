#!/usr/bin/env python3
"""Cross-platform installation and video generation for the Accio Video Skill."""
...
PROJECT_ARCHIVE_URL = (
    "https://github.com/leogoecking/Accio-Video/archive/refs/heads/main.zip"
)
DEFAULT_ROOT = Path.home() / "Accio Video"
...
RECOMMENDED_LLM_PROVIDERS = {
    "moonshot": (
        "Kimi / Moonshot AI",
        "https://platform.kimi.com?track_id=track-2f5441d6ffd84c509dd079d78e9db5dc",
    ),
    ...
    "volcengine": (
        ...
        "https://www.volcengine.com/activity/ai618",
    ),
...
def log(message: str) -> None:
    ...
    print(f"[Accio Video] {message}", flush=True)


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Install Accio Video and generate a final video from a topic."
    )
    ...
    parser.add_argument(
        ...
        help=f"Accio Video installation directory (default: {DEFAULT_ROOT})",
    )
    parser.add_argument(
        "cli_args",
        nargs=argparse.REMAINDER,
        help="additional Accio Video CLI arguments placed after --",
    )
...
def ensure_project(root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="mpt-install-") as temp_dir_value:
        ...
        archive_path = temp_dir / "Accio Video.zip"
        request = urllib.request.Request(
            PROJECT_ARCHIVE_URL,
            headers={"User-Agent": "Accio Video-Agent-Skill"},
        )
        ...
        if len(candidates) != 1:
            raise SkillError("download completed but no valid Accio Video project was found")
        if root.exists():
            root.rmdir()
...
def _validate_pexels_key(api_key: str) -> str:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "Accio Video-Agent-Skill",
        },
    )
    try:
...
def result_manifest_path(root: Path) -> Path:
    return root / ".agent-logs" / "accio-video-video" / "latest-result.json"
...
def generate_video(
    ...
    log_dir = root / ".agent-logs" / "accio-video-video"
