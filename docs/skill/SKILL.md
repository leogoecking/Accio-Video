---
name: accio-video
description: Use this skill whenever the user wants to create a finished video from a topic, title, idea, prompt, or script with Accio Video. This includes short-form, voice-over, educational, marketing, social-media, and stock-footage videos. Also use it when the user mentions Accio Video, provides this Skill URL, asks an AI agent to install or configure Accio Video, needs missing API keys identified, wants a failed generation repaired, or wants a generated MP4 located and delivered. Use this skill when the expected outcome is a final video file, not setup instructions.
...
metadata:
  author: "leogoecking@hotmail.com"
  version: "1.3.2"
  upstream: "https://github.com/leogoecking/Accio-Video"
---

# Accio Video Video Generation
...
Unless the user requests otherwise, generate one Chinese `9:16` portrait video with Pexels footage, the default Chinese Edge TTS voice, subtitles, and background music. Install Accio Video under the user's home directory.
...
```text
https://raw.githubusercontent.com/leogoecking/Accio-Video/main/docs/skill/mpt_agent.py
```
...
```text
~/Accio Video/.agent-logs/accio-video/latest-result.json
```
...
- Use uv and the Accio Video CLI only.
