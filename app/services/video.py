...
def combine_videos(
    ...
    if scene_durations and len(scene_durations) == len(video_paths):
        ...
        try:
            ...
            if processed_clips:
                delete_files([c.file_path for c in processed_clips])
...
def generate_video(
    ...
    # https://github.com/leogoecking/Accio-Video/issues/217
        ...
        try:
            ...
            if ext in const.FILE_TYPE_IMAGES:
                ...
                try:
                    ...
                    logger.debug(f"ffmpeg image render fallback to moviepy: {exc}")
                    rendered_with_ffmpeg = False
